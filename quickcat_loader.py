...
Scripts that import from *other skill packages* must also call the appropriate
registration helper before their cross-skill imports:

    import quickcat_loader
    quickcat_loader.register_tie_breaker()     # harvest_orchestrator.py
    quickcat_loader.register_batch_cleaner()   # import_pipeline.py
    quickcat_loader.register_marc_importer()   # import_pipeline.py
...
# ── Eager layer: shared-resources (no cross-deps, no heavy third-party libs) ──
...
# ── Eager layer: copy-cataloger base (no pandas, needed by most copy-cat scripts)
...
def register_tie_breaker() -> None:
    """Register resolve_tie_breaker for scripts that import it (e.g. harvest_orchestrator).

    Kept out of the eager layer because resolve_tie_breaker.py itself imports
    quickcat_loader; eager registration would double-execute it when it runs as __main__.
    """
